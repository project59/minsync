  ...
  return (
    <div className="bg-white dark:bg-slate-900 border-primary p-5 mb-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-blue-50 dark:bg-slate-900 p-4 text-center">
          <div className="flex items-center justify-center gap-2 mb-1">
            <FilePlus className="w-5 h-5 text-blue-600" />
            <span className="text-2xl font-bold text-blue-600">{newFiles.length}</span>
          </div>
          <span className="text-xs text-slate-500 dark:text-slate-400">New files to copy</span>
          {newFiles.length > 0 && (
            <div className="text-xs text-blue-600 mt-1 font-medium">{sizeMB} MB</div>
          )}
        </div>
        <div className="bg-green-50 dark:bg-slate-900 p-4 text-center">
          <div className="flex items-center justify-center gap-2 mb-1">
            <CheckCircle className="w-5 h-5 text-green-600" />
            <span className="text-2xl font-bold text-green-600">{existingFiles.length}</span>
          </div>
          <span className="text-xs text-slate-500 dark:text-slate-400">Already on PC</span>
        </div>
      </div>
